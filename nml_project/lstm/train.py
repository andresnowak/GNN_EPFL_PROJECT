...
if log_wandb:
    wandb.init(project="eeg-lstm", config={
        "epochs": epochs,
...
best_f1 = 0.0
if log_wandb:
    ckpt_path = os.path.join(wandb.run.dir, "best_lstm_model.pth")
else:
    ckpt_path = os.path.join(script_dir, "best_lstm_model.pth")
global_step = 0
...
# Save the model
if log_wandb:
    torch.save(model.state_dict(), os.path.join(wandb.run.dir, "final_lstm_model.pth"))
else:
    torch.save(model.state_dict(), os.path.join(script_dir, "final_lstm_model.pth"))
