...
if log_wandb:
    wandb.init(project="eeg-lstm-gcn", config={
        "epochs": epochs,
...
print('Number of parameters in GCN:', sum(p.numel() for p in model.gcn1.parameters() if p.requires_grad) + sum(p.numel() for p in model.gcn2.parameters() if p.requires_grad) + sum(p.numel() for p in model.gcn3.parameters() if p.requires_grad))
...
if log_wandb:
    ...
    ckpt_path = os.path.join(script_dir, "best_lstm_gcn_model.pth")
global_step = 0
...
if log_wandb:
    ...
    torch.save(model.state_dict(), os.path.join(script_dir, "final_lstm_gcn_model.pth"))
