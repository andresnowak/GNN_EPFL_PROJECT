...
if log_wandb:
    wandb.init(project="eeg-s4-gcn", config={
        "epochs": epochs,
...
model = S4_GCN(s4_hidden_dim, s4_dropout, gcn_hidden, gcn_out, dropout).to(device)
...
best_f1 = 0.0
if log_wandb:
    ckpt_path = os.path.join(wandb.run.dir, "best_s4_gcn_model.pth")
else:
    ckpt_path = os.path.join(script_dir, "best_s4_gcn_model.pth")
global_step = 0
...
# Save the model
if log_wandb:
    torch.save(model.state_dict(), os.path.join(wandb.run.dir, "final_s4_gcn_model.pth"))
else:
    torch.save(model.state_dict(), os.path.join(script_dir, "final_s4_gcn_model.pth"))
