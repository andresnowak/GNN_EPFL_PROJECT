...
wd = 1e-5
s4_hidden_dim = 128
gat_hidden, gat_out, num_heads, gat_dropout = 256, 256, 4, 0.2

# Initialize Weights and Biases
if log_wandb:
    wandb.init(project="eeg-s4-gat", config={
        "epochs":epochs, "batch_size":batch_size,
        "lr":lr, "weight_decay":wd,
        "s4_hidden_dim":s4_hidden_dim,
        "gat_hidden":gat_hidden, "gat_out":gat_out,
        "num_heads":num_heads, "dropout":gat_dropout,
    })

model = S4_GAT(s4_hidden_dim, gat_hidden, gat_out, num_heads, gat_dropout).to(device)
print("Params:", sum(p.numel() for p in model.parameters() if p.requires_grad))

neg,pos = clips_tr['label'].value_counts()[0], clips_tr['label'].value_counts()[1]
...
best_f1 = 0.0
if log_wandb:
    ckpt_path = os.path.join(wandb.run.dir, "best_s4_gat_model.pth")
else:
    ckpt_path = os.path.join(script_dir, "best_s4_gat_model.pth")
global_step = 0
...
# Save the model
if log_wandb:
    torch.save(model.state_dict(), os.path.join(wandb.run.dir, "final_s4_gat_model.pth"))
else:
    torch.save(model.state_dict(), os.path.join(script_dir, "final_s4_gat_model.pth"))
