...
if log_wandb:
    wandb.init(project="eeg-cnn-gat", config={
        "epochs": epochs,
...
model = CNN_GAT(cnn_out_dim, gat_hidden, gat_out, num_heads, dropout).to(device)
print('Number of trainable parameters:', sum(p.numel() for p in model.parameters() if p.requires_grad))
print('Number of parameters in CNN:', sum(p.numel() for p in model.cnn.parameters() if p.requires_grad))
print('Number of parameters in Linear after CNN:', sum(p.numel() for p in model.fc_cnn.parameters() if p.requires_grad))
...
best_f1 = 0.0
if log_wandb:
    ckpt_path = os.path.join(wandb.run.dir, "best_cnn_gat_model.pth")
else:
    ckpt_path = os.path.join(script_dir, "best_cnn_gat_model.pth")
global_step = 0
...
# Save the model
if log_wandb:
    torch.save(model.state_dict(), os.path.join(wandb.run.dir, "final_cnn_gat_model.pth"))
else:
    torch.save(model.state_dict(), os.path.join(script_dir, "final_cnn_gat_model.pth"))
