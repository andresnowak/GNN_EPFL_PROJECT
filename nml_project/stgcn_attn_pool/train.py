...
from models import get_normalized_adj, STGCNClassifier_AttnPool
...
def main(config: dict):
    ...
    dataset_tr = AugmentedDataset(dataset_tr, config["training"]["augment"], config["training"]["augment_values"])
    ...
    if log_wandb:
        ...
        save_config(config, wandb_dir, "stgcn_3.yaml")
    else:
        save_config(config, output_dir, "stgcn_3.yaml")
    ...
    if log_wandb:
        ...
        ckpt_path = os.path.join(output_dir, config["checkpoint"]["best_model_filename"])
    ...
    if log_wandb:
        ...
        torch.save(
                model.state_dict(),
                os.path.join(output_dir, config["checkpoint"]["final_model_filename"]),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", required=False, help="Path to yaml config file", default=os.path.join(script_dir, "configs/stgcn_3.yaml"))
