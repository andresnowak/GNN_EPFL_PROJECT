...
class TGCNWrapper(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        H_states = torch.zeros(seq_len, num_nodes, self.hidden_dim, device=x.device)
        H_2_states = torch.zeros(seq_len, num_nodes, self.hidden_dim, device=x.device)

        for b in range(batch_size):
            H = torch.zeros(num_nodes, self.hidden_dim, device=x.device)
            H_2 = torch.zeros(num_nodes, self.hidden_dim, device=x.device)

            for t in range(seq_len):
                x_t = x[b, :, t].unsqueeze(-1)  # [num_nodes, 1]
                
                # Update hidden states
                H = self.tgcn_1(x_t, edge_index, edge_weight, H)
                H_2 = self.tgcn_2(H, edge_index, edge_weight, H_2)
                
                # Store states directly in pre-allocated tensors
                H_states[t] = H
                H_2_states[t] = H_2

            # Compute mean over time (no stacking needed)
            H_avg = H_states.mean(dim=0)  # [num_nodes, hidden_dim]
            H_2_avg = H_2_states.mean(dim=0)
