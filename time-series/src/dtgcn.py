...
class DTGCN(nn.Module):
    def __init__(
        ...
        self.tgcn = TGCN(window_size, tgcn_hidden_dim)  # accepts at forward always a different graph adjacency and list
    ...
    def forward(self, x, static_edge_index, static_edge_weight=None):
        ...
        for t in range(num_windows):
            ...
            for i in range(batch_size):
                data_list.append(
                    Data(x=I_t[i], edge_index=static_edge_index, edge_weight=static_edge_weight)
                )
            ...
            for i in range(batch_size):
                data_list.append(
                    Data(
                        x=v_t_step[i],
                        edge_index=batched_edge_index[i],
                        edge_weight=batched_edge_weight[i],
                    )
                )
            batch_graph = Batch.from_data_list(data_list)
