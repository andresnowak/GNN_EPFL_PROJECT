...
class TGCN(torch.nn.Module):
    ...
    def _create_update_gate_parameters_and_layers(self):
        ...
        # Dense GCNCOnv has the self-loop in the forward function

        self.linear_z = torch.nn.Linear(2 * self.out_channels, self.out_channels)

    def _create_reset_gate_parameters_and_layers(self):

        self.conv_r = DenseGCNConv(
            in_channels=self.in_channels,
            out_channels=self.out_channels,
            improved=self.improved,
            # cached=self.cached,
    ...
    def _calculate_update_gate(self, X, adj, H):
        Z = torch.cat([self.conv_z(X, adj), H], axis=1)
        Z = self.linear_z(Z)
        Z = torch.sigmoid(Z)
        return Z

    def _calculate_reset_gate(self, X, adj, H):
        R = torch.cat([self.conv_r(X, adj), H], axis=1)
        R = self.linear_r(R)
        R = torch.sigmoid(R)
        return R

    def _calculate_candidate_state(self, X, adj, H, R):
        H_tilde = torch.cat([self.conv_h(X, adj), H * R], axis=1)
